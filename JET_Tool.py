...
#JET 단계별 분류와 각 단계별 파트내 문항수 및 배점 할당
LEVELS = {
    "초급": {
...
# ====== 여기만 채워두면 됨: 정답 키 ====== 답지 내용에 따라 매달 바꿀것
ANSWER_KEYS = {
    "초급": {
        "LC": {
            "파트1": ["2","2","1","2","1","1"],  # 예: ["A","C","B","D","..."] (6개)
            "파트2": ["2","3","1","1","3","3","2","2","3","2","1","1"],
            "파트3": ["2","1","1","3","2","3","1","1","2","3"],
            "파트4": ["1", "2", "1", "1", "3", "1", "3", "3"],
            "파트5": ["3", "2", "1", "3"],
        },
        "RC": {
...
def main():
    print("=== 교사용 JET 고급, 중급, 초급  채점 툴 === (종료: q)")
    # 정답 키 길이 사전 점검
    try:
        check_key_lengths()
